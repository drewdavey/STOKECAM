...
trimFlag = 1;       % trim to refined polygon? else basic QC and trim to bbox  
...
while cleanFlag
    ...
    if trimFlag
        % Trim to polygon
        [data.points3D, data.colors, data.J1] = refinedTrim(data.points3D, data.colors, data.J1);
    else
        % Trim to bounds
    ...
    if colorFlag
        [data.points3D, data.colors, data.J1] = colorThresh(data.points3D, data.colors, data.J1);
    end
