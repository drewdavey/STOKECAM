...
def configure_cameras(fname_log, mode, exposure_us):
    ...
    tstr = datetime.now(timezone.utc).strftime('%H%M%S%f')
    log = open(fname_log, 'a')
    log.write(f"{tstr}:     Configuring cameras to {mode} mode with {exposure_us} µs exposure...\n")
...
trigger = DigitalOutputDevice(trigger_pin, active_high=False, initial_value=False)
