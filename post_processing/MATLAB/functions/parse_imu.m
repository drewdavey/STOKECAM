...
function imu = parse_imu(mainDir, waveFolder)
    ...
    str = arrayfun(@(h, m, s, f) sprintf('%02d:%02d:%02d.%03d', h, m, floor(s), round(f)), ...
               vn.utcHour, vn.utcMinute, vn.utcSecond, vn.utcFracsec, 'UniformOutput', false);
    vn.utcTimes = datetime(str, 'Format', 'HH:mm:ss.SSS', 'InputFormat', 'HH:mm:ss.SSS');
